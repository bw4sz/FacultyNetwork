...
dbGetQuery(d$con, "CREATE TABLE m 
           AS SELECT DOI, Citations, Year, MAX (`Order`,Author) as AuID FROM Meta")

#we want to remove authors that have few publications, atleast 4

sq<-"SELECT Author, Count(*) as p FROM Meta GROUP BY Author HAVING p > 4"
auth<-dbGetQuery(d$con, sq)

tocompare<-d %>% tbl("Meta") %>% filter(Author %in% auth$Author) %>% distinct() %>% collect()

#seems to be an error that if there is just 1 author, the unique ID gets screwed up.
