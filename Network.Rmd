...
Links<-tmod[,c("To","From","estimate")]
#list of nodes
a<-data.frame(name=unique(c(tmod$To,tmod$From)))
Links$source<-sapply(Links$To,function(x) which(x ==a$name))
Links$target<-sapply(Links$From,function(x) which(x ==a$name))
Links$value<-Links$estimate

sankeyNetwork(Links = Links, Nodes = data.frame(name=a),
...
MisNodes<-data.frame(name=sort(as.character(unique(c(MisLinks$To,MisLinks$From)))))
#Add groups
MisNodes$Group=1

MisLinks$source<-sapply(MisLinks$To,function(x) which(x ==MisNodes$name))
MisLinks$target<-sapply(MisLinks$From,function(x) which(x ==MisNodes$name))

#Order by source
MisLinks<-MisLinks[order(MisLinks$source),]

#remove NA's
MisLinks<-MisLinks[!is.na(MisLinks$value),]

d3SimpleNetwork(MisLinks,height=500,width=700)  %>% saveNetwork(file = 'Net1.html')
d3SimpleNetwork(MisLinks,height=500,width=700)
```

##Force based network

```{r}
d3Network::d3ForceNetwork(Links = MisLinks, Nodes = MisNodes,Source = "source", Target = "target",Value = "value", Group = "Group" ,NodeID = "name", width = 700, height = 800)
```
