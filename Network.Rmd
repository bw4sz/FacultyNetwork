...
dbGetQuery(d$con,"UPDATE m SET Author = 'm.Order' WHERE AUTHOR < 100 ")

#we want to remove authors that have few publications, atleast 4
sq<-"SELECT Author, Count(*) as p FROM m GROUP BY Author HAVING p > 4"
auth<-dbGetQuery(d$con, sq)

tocompare<-d %>% tbl("m") %>% filter(Author %in% auth$Author) %>% distinct() %>% collect()
...
```{r}
j_class<-dbGetQuery(d$con,"SELECT j_class.Discipline,J_class.Class,J_class.Publication FROM j_class,JA WHERE 'j_class.Publication' = 'JA.Journal'")
