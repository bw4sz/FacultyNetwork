...
Let's begin slowly. I want to search an affiliation for any of the journal titles i have in my categories. I found the full_text package and i want to give it a try. 

```{r}
q <- "Ben G. Weinstein"
s<-ft_search(q, limit=500, plosopts=list(fl=c('author','journal')))

```

#Find authors

The simplest thing to do here is to split the author lists into pieces and do a frequency table to find potential authors.

```{r}
s$crossref$data$container.title

```

###Alternative strategy

Try search first by journal name and matching it to our journal lists

```{r}
jout<-lapply(j_class$Publication[1:10],function(x){
out <- ft_providers(as.character(x))$data
})

#remove null results
jout<-jout[sapply(jout,nrow)>0]
jout<-rbind_all(jout)
```

#Find papers from those journals

```{r}
getj<-cr_journals(issn=jout[1,]$issn)
...
```{r, include=FALSE}
   ...
   file.rename(from="FacultyNetwork.md", 
               to="README.md")
```
