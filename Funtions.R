...
scquery <- function(inquery,year) {
  ...
  inst.token <-
    readLines("C:/Users/Ben/Dropbox/FacultyNetwork/InstToken.txt")
  apiKey <-
    readLines("C:/Users/Ben/Dropbox/FacultyNetwork/apikey.txt")
  
  #format string
  str <-
    "https://api.elsevier.com/content/search/scopus?&httpAccept=application/xml&view=complete&count=100"
  
  #fields
  f <-
    "field=prism:publicationName,dc:title,dc:creator,citedby-count,prism:coverDate,author,dc:identifier"
...
sc_parse <- function(response) {
  ...
  ns <-
    structure(sapply(nsDefs, function(x)
      x$uri), names = names(nsDefs))
  names(ns)[1] <- "xmlns"
  
  #set namespaces
  e <-
    xpathSApply(xmltop,"//xmlns:entry/xmlns:error",xmlValue,namespaces = ns)
  if (length(e) == 1) {
    return(NA)
  }
  ...
  ##Get journal
  journal <-
    xpathSApply(xmltop,"//prism:publicationName",xmlValue,namespaces = ns)
  
  ##All authors
  #how many articles are there?
  lresponse <-
    length(getNodeSet(xmltop,"//xmlns:entry",namespaces = ns,xmlValue))
  ...
  for (x in 1:lresponse) {
    ...
    xpath <-
      paste("//xmlns:entry[",x,"]//xmlns:author/xmlns:authid",sep = "")
    allauthors[[x]] <-
      as.list(xpathSApply(xmltop,xpath,xmlValue,namespaces = ns))
  }
  
  names(allauthors) <-
    xpathSApply(xmltop,"//xmlns:entry//dc:identifier",xmlValue,namespaces =
                  ns)
  ...
  citation <-
    as.numeric(
      xpathSApply(
        xmltop,"//xmlns:entry//xmlns:citedby-count",xmlValue,namespaces = ns
      )
    )
  ...
  Year <-
    years(xpathSApply(
      xmltop,"//xmlns:entry//prism:coverDate",xmlValue,namespaces = ns
    ))
  ...
  artdf <-
    data.frame(
      Journal = journal,Citations = citation,Year = Year,DOI = DOI
    )
  ...
  if (nrow(artmatch) == 0) {
    artmatch <-
      artdf[paste("The",artdf$Journal) %in% j_class$Publication,]
  }
...
getCount <- function(response) {
  ...
  #Get total results
  tresults <-
    as.numeric(xpathSApply(xmltop,"//opensearch:totalResults",xmlValue,namespaces =
                             ns))
  return(tresults)
}
...
currentCount <- function(response) {
  ...
  tresults <-
    length(xpathSApply(xmltop,"//xmlns:entry",xmlValue,namespaces = ns))
  return(tresults)
}
...
getSourceID <- function(inquery) {
  ...
  #format string
  toget <-
    "https://api.elsevier.com/content/serial/title?&httpAccept=application/xml&count=100"
...
parseSource <- function(response,inquery) {
  ...
  r <-
    r[toupper(r$title) %in% toupper(gsub(inquery,pattern = "\\+",replacement =
                                           " ")),]
  return(r)
}
...
GeomBar_horz <- proto(ggplot2:::Geom, {
  ...
  default_aes <-
    function(.)
      aes(
        colour = NA, fill = "grey20", size = 0.5, linetype = 1, weight = 1, alpha = NA
      )
...
calcN <- function(x) {
  ...
  E(g)$color <-
    rgb(0,1,0,alpha = E(g)$weight / max(E(g)$weight),maxColorValue = 1)
  
  ramp <- colorRamp(c("blue","red"),alpha = T)
  
  E(g)$color = apply(ramp(E(g)$weight), 1, function(x)
    rgb(x[1] / 255,x[2] / 255,x[3] / 255,alpha = T))
  ...
  #label sizes
  V(g.copy)$label.cex <-
    V(g.copy)$degree / max(V(g.copy)$degree) * 1 + .2
  ...
  vdat <-
    data.frame(
      Class = names(between_class),Between = between_class,Degree = degree_class,Closeness =
        closeness_class,Eigen = eigenV
    )
  return(vdat)
}
CalcG <- function(x) {
...
shead <-
  function(tab) {
    dbGetQuery(d$con,paste("SELECT * FROM",tab,"limit 10"))
  }
...
queryscopus <- function(runs = 20,size = 20) {
  ...
  if (tq$status_code == 200) {
    dat <-
      foreach(
        x = jp,.errorhandling = "pass",.packages = c(
          "httr","XML","proto","reshape2","plyr","dplyr","chron","stringr"
        ),.export = "journaldf"
      ) %dopar% {
        #get functions
        source("Funtions.R")
    ...
    towrite <-
      df %>%  distinct(DOI) %>% select(DOI,Author,Order,Citations,Year)
